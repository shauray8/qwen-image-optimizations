...
class QwenImagePipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    ...
    def __call__(
        ...
        # Pad embeddings once before loop for batched CFG
        if do_true_cfg:
            ...
            if prompt_embeds.shape[1] < max_seq_len:
                ...
                prompt_embeds = torch.cat([
                    prompt_embeds,
                    torch.zeros(prompt_embeds.shape[0], pad_len, prompt_embeds.shape[2], 
                               device=prompt_embeds.device, dtype=prompt_embeds.dtype)
                ], dim=1)
                prompt_embeds_mask = torch.cat([
                    prompt_embeds_mask,
                    torch.zeros(prompt_embeds_mask.shape[0], pad_len, 
                               device=prompt_embeds_mask.device, dtype=prompt_embeds_mask.dtype)
                ], dim=1)
            
            # Pad negative_prompt_embeds if needed
            if negative_prompt_embeds.shape[1] < max_seq_len:
                pad_len = max_seq_len - negative_prompt_embeds.shape[1]
                negative_prompt_embeds = torch.cat([
                    negative_prompt_embeds,
                    torch.zeros(negative_prompt_embeds.shape[0], pad_len, negative_prompt_embeds.shape[2],
                               device=negative_prompt_embeds.device, dtype=negative_prompt_embeds.dtype)
                ], dim=1)
                negative_prompt_embeds_mask = torch.cat([
                    negative_prompt_embeds_mask,
                    torch.zeros(negative_prompt_embeds_mask.shape[0], pad_len,
                               device=negative_prompt_embeds_mask.device, dtype=negative_prompt_embeds_mask.dtype)
                ], dim=1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_true_cfg:
                    ...
                    batched_timestep = torch.cat([timestep, timestep], dim=0) / 1000
                    batched_embeds = torch.cat([prompt_embeds, negative_prompt_embeds], dim=0)
                    batched_embeds_mask = torch.cat([prompt_embeds_mask, negative_prompt_embeds_mask], dim=0)
                    batched_img_shapes = img_shapes + img_shapes
                    batched_guidance = torch.cat([guidance, guidance], dim=0) if guidance is not None else None
                    ...
                    with self.transformer.cache_context("batched_cfg"):
                        ...
                        batched_noise_pred = self.transformer(
                            ...
                            encoder_hidden_states_mask=batched_embeds_mask,
                            encoder_hidden_states=batched_embeds,
                            img_shapes=batched_img_shapes,
