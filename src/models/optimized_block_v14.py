"""
V14: Fused QKV-split + RMSNorm + RoPE + joint-pack kernel.
"""
...
from models.fused_qkv_fp8 import FusedQKVFP8Linear, fuse_qkv_in_attention
...
from kernels.fused_kernels_v3 import fused_qkv_rmsnorm_rope_pack
...
def _fp8_gemm(x_fp8, x_scale, fp8_linear):
    return torch._scaled_mm(
        ...
        bias=fp8_linear.bias,
    )

class FusedQKVFP8LinearRaw(FusedQKVFP8Linear):
    def forward_with_fp8_input_raw(self, x_fp8, a_scale):
        """Return full [M, 3*H*D] contiguous output for use with fused pack kernel."""
        ...
        # output is [M, 3*H*D], contiguous (stride=(3*H*D, 1))
        return output

@maybe_allow_in_graph
class FusedQwenImageTransformerBlockV14(nn.Module):
    ...
    def forward(
        ...
        fused_qkv_rmsnorm_rope_pack(
            ...
            joint_seq, txt_seq,   # joint_offset = txt_seq for img
            eps=self.eps,
        )
        # Txt tokens → joint[:txt_seq]
        fused_qkv_rmsnorm_rope_pack(
            txt_qkv_raw, joint_q_buf, joint_k_buf, joint_v_buf,
            attn.norm_added_q.weight, attn.norm_added_k.weight,
            txt_cos, txt_sin,
            B, txt_seq, heads, head_dim,
            joint_seq, 0,         # joint_offset = 0 for txt
            eps=self.eps,
        )

        # Flash attention on fully-assembled joint buffers
        joint_out = flash_attn_wrapper(
            joint_q_buf, joint_k_buf, joint_v_buf,
            softmax_scale=1.0 / 11.313708498984761,
        )[0]
        joint_out = joint_out.flatten(2, 3).to(joint_q_buf.dtype)  # [B, joint_seq, D]

        txt_attn = joint_out[:, :txt_seq, :]
        img_attn = joint_out[:, txt_seq:, :]

        img_attn = attn.to_out[0](img_attn)
...
def _fuse_qkv_raw(attn_module):
    """
    Create FusedQKVFP8LinearRaw for both img and txt streams.
    Returns (img_qkv, txt_qkv).
    """
    def _make_raw(q_lin, k_lin, v_lin):
        base = FusedQKVFP8LinearRaw(q_lin, k_lin, v_lin)
        return base
    ...
    if (isinstance(attn_module.to_q, OptimizedRowwiseFP8Linear) and
        ...
        img_qkv = _make_raw(attn_module.to_q, attn_module.to_k, attn_module.to_v)
    else:
        img_qkv = None
    ...
    if (hasattr(attn_module, 'add_q_proj') and
        ...
        txt_qkv = _make_raw(attn_module.add_q_proj, attn_module.add_k_proj, attn_module.add_v_proj)
    else:
        txt_qkv = None
...
def _patch_rope_decomposition(model):
    original_forward = model.pos_embed.forward

    def patched(*args, **kwargs):
        img_freqs, txt_freqs = original_forward(*args, **kwargs)
        return (img_freqs.real.contiguous(), img_freqs.imag.contiguous(),
                txt_freqs.real.contiguous(), txt_freqs.imag.contiguous())

    model.pos_embed.forward = patched

def _v14_forward_corrected(
    self,
    hidden_states,
    ...
    from transformer import compute_text_seq_len_from_mask
    from fused_kernels import fused_adaln_fp8
...
def apply_v14_optimizations(model, verbose=True):
    import types
    from models.transformer import QwenImageTransformerBlock

    if verbose:
        print("V14: Fused QKV-split + RMSNorm + RoPE + joint-pack kernel")
    replaced = 0
    for name, module in list(model.named_modules()):
        if isinstance(module, QwenImageTransformerBlock):
            new_block = FusedQwenImageTransformerBlockV14(
                dim=module.dim,
            ...
            if img_qkv is None:
                raise RuntimeError(
                    f"Block {name}: img Q/K/V are not OptimizedRowwiseFP8Linear. "
                    "Run apply_optimized_rowwise_fp8 first."
                )
            if txt_qkv is None:
                raise RuntimeError(
                    f"Block {name}: txt Q/K/V are not OptimizedRowwiseFP8Linear. "
                    "Run apply_optimized_rowwise_fp8 first."
                )
    ...
    if verbose:
        print(f"  Replaced {replaced} blocks with V14 blocks")

    _patch_rope_decomposition(model)
    model.forward = types.MethodType(_v14_forward_corrected, model)
